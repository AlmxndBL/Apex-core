...
* ❌ **ห้ามรัน Full Build (`npm run build` / `next build` / `nuxt build`)** ทุกครั้งที่แก้โค้ดเล็กๆ เพราะเสียเวลาโดยใช่เหตุ
* ✅ **Fast In-Memory TypeCheck (1-3 วินาที):**
  * **Nuxt / Vue 3:** `npx vue-tsc --noEmit`
  * **React / Next.js:** `npx tsc --noEmit`
* ✅ **Targeted Logic Test:** รันเทสต์เฉพาะไฟล์ที่แก้ไข (`npx vitest run path/to/test.spec.ts`)
